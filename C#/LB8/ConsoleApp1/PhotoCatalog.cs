...
public class PhotoCatalog
{
        ...
        try
        {
            if (File.Exists(FilePath))
            {
                ...
                using (var reader = new BinaryReader(fs, Encoding.UTF8))
                {
                    ...
                    {
                        var photo = new Photo();
                        photo.SetID(reader.ReadInt32());
                        photo.SetTitle(reader.ReadString());
                        photo.SetDateTaken(new DateTime(reader.ReadInt64()));
                        photo.SetFileSize(reader.ReadInt64());
                        photo.SetResolution(reader.ReadString());
                        photos.Add(photo);
                    }
                }
                return photos;
    ...
    public void SaveToFile(List<Photo> photos)
    {
        try
        {
            ...
            using (var writer = new BinaryWriter(fs, Encoding.UTF8))
            {
                ...
                {
                    writer.Write(photo.GetID());
                    writer.Write(photo.GetTitle());
                    writer.Write(photo.GetDateTaken().Ticks);
                    writer.Write(photo.GetFileSize());
                    writer.Write(photo.GetResolution());
                }
            }
    ...
    {
        var photoToRemove = photos.FirstOrDefault(p => p.GetID() == id);
    ...
    {
        return photos.Where(p => p.GetDateTaken() > date).ToList();
    }

    public List<Photo> GetPhotosLargerThanSize(List<Photo> photos, long size)
    {
        return photos.Where(p => p.GetFileSize() > size).ToList();
    }
    ...
    public double GetAverageFileSize(List<Photo> photos)
    {
        ...
        return photos.Average(p => p.GetFileSize());
    }
}
